<template>
  <div>
    <div class="container">
      <div class="h1">Create Jackpot</div>
      ...
      <div class="popup" v-if="success">
        ...
        <div class="content">
          ...
          <p>your cagnotte has been created successfully</p>
        </div>
      </div>
      <div class="popup" v-if="fail">
        ...
        <div class="content">
          <button class="close-btn" @click="closePop">&times;</button>
          <p v-if="code ==18"> This name is already used, try using another name for your Jackpot</p>
...
<script>
import { TxCreateCagnotte } from "../static/blockchainMethods";
  ...
  methods: {
    onSubmit() {
      ...
      if (!this.cagnotteName) {
        alert("add the jackpot name");
        return;
      }

      
      this.loader = true;
      TxCreateCagnotte(this.$store.state.client, this.cagnotteName).then(
        (response) => {
          if (response.code) {
